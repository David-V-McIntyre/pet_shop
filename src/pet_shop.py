...
def get_customer_cash(a):
    return a["cash"]
            
def remove_customer_cash(a,b):
    a["cash"] -= b
    return a["cash"]

def get_customer_pet_count(a):
    cust_pets = len(a["pets"])
    return cust_pets

def add_pet_to_customer(a,b):
    a["pets"].append(b)

def customer_can_afford_pet(a,b):
    if a["cash"] >= b["price"]:
        return True
    else:
        return False
